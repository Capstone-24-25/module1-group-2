...
# we are doing this at the very beginning instead of at the end
set.seed(101422)
...
# Up until here everything has been basically the same as in class
# However, now I am doing the "fuzzy" selection:
...
variable_choices <- ttest_results %>%
    left_join(rf_importance, by = "protein")

variable_choices <- variable_choices %>% mutate(
...
proteins_sstar <- variable_choices %>%
...
biomarker_sstar <- training_biomarker %>%
